...
**Number of animals with gas records: `r if ((nrow(RFID_file) > 0)) { paste0(nrow(dplyr::semi_join(RFID_file, df, by = "RFID")), " out of ", nrow(RFID_file)) } else { length(unique(df$RFID)) }`**
...
if (nrow(RFID_file) > 0) {
  ...
  print("The 'RFID_file' is not provided.")
}
```
...
plot1 <- ggplot(as.data.frame(table(as.Date(df$StartTime))), aes(x = Var1, y = Freq)) +
  ...
  labs(title = "Total Records Per Day", 
       x = "", 
       y = "Total records") +
...
plot2 <- df %>%
  ...
  labs(title = "Normalized Gas Production Per Day", 
       x = "", 
...
group_var <- if (nrow(RFID_file) > 0) "FarmName" else "RFID"
...
plot1 <- df %>%
    ...
    labs(title = "Total Records Per Animal", 
         x = "", 
         y = "Total Records") +
    theme_classic() +
    theme(
...
plot2 <- df %>%
    ...
    labs(title = "Daily Records Distribution", 
         x = "", 
    ...
    p1 <- df %>%
      ...
      dplyr::summarise(daily_CH4 = weighted.mean(CH4GramsPerDay, GoodDataDuration, na.rm = TRUE)) %>%
      {
        ggplot(., aes(x = reorder(!!sym(group_var), -daily_CH4), y = daily_CH4, color = daily_CH4)) +
          ...
          labs(title = "Methane (CH4) Production Per Animal", 
               x = "", 
               y = "CH4 (g/d)") +
          theme_classic() +
          theme(
      ...
      dplyr::summarise(daily_CO2 = weighted.mean(CO2GramsPerDay, GoodDataDuration, na.rm = TRUE)) %>%
      {
        ggplot(., aes(x = reorder(!!sym(group_var), -daily_CO2), y = daily_CO2, color = daily_CO2)) +
          ...
          stat_summary(fun = mean, geom = "errorbar", 
                       aes(ymax = ..y.., ymin = ..y..), width = 0.75, size = 0.7, 
                       color = "black", linetype = "solid") +
          labs(title = "Carbon Dioxide (CO2) Production Per Animal", 
               x = "", 
               y = "CO2 (g/d)") +
          theme_classic() +
          theme(
      ...
      dplyr::summarise(daily_O2 = weighted.mean(O2GramsPerDay, GoodDataDuration, na.rm = TRUE)) %>%
      {
        ggplot(., aes(x = reorder(!!sym(group_var), -daily_O2), y = daily_O2, color = daily_O2)) +
          ...
          stat_summary(fun = mean, geom = "errorbar", 
                       aes(ymax = ..y.., ymin = ..y..), width = 0.75, size = 0.7, 
                       color = "black", linetype = "solid") +
          labs(title = "Oxygen (O2) Production Per Animal", 
               x = "", 
               y = "O2 (g/d)") +
          theme_classic() +
          theme(
        ...
        dplyr::summarise(daily_H2 = weighted.mean(H2GramsPerDay, GoodDataDuration, na.rm = TRUE)) %>%
        {
          ggplot(., aes(x = reorder(!!sym(group_var), -daily_H2), y = daily_H2, color = daily_H2)) +
            ...
            stat_summary(fun = mean, geom = "errorbar", 
                         aes(ymax = ..y.., ymin = ..y..), width = 0.75, size = 0.7, 
                         color = "black", linetype = "solid") +
            labs(title = "Hydrogen (H2) Production Per Animal", 
                 x = "", 
                 y = "H2 (g/d)") +
            theme_classic() +
            theme(
