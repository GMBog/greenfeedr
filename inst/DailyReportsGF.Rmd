...
# Determine the order of FarmName based on daily_CH4
farmname_order <- summarize_data(df) %>%
  dplyr::arrange(desc(daily_CH4)) %>%
  dplyr::pull(FarmName)

# Plot 1: Total number of records per cow
plot1 <- summarize_data(df) %>%
...
plot2 <- function(df, farmname_order) {
  df %>%
    ...
    ggplot(aes(x = factor(FarmName, levels = farmname_order), y = n, fill = factor(AMPM, levels = c("10PM-4AM", "4AM-10AM", "10AM-4PM", "4PM-10PM")))) +
...
generate_plots <- function(data, Plot_opt = c("All", "CH4", "O2", "CO2", "H2")) {
  ...
  if ("co2" %in% options_selected) {
    p3 <- df %>%
      ...
      {
        ggplot(., aes(x = reorder(FarmName, -daily_CO2), y = daily_CO2, color = daily_CO2)) +
          geom_boxplot(fatten = NULL, outlier.shape = NA) +
          stat_summary(fun = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..), width = 0.75, size = 0.7, color = "black", linetype = "solid") +
  ...
  if ("o2" %in% options_selected) {
    p2 <- df %>%
      ...
      {
        ggplot(., aes(x = reorder(FarmName, -daily_O2), y = daily_O2, color = daily_O2)) +
          geom_boxplot(fatten = NULL, outlier.shape = NA) +
          stat_summary(fun = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..), width = 0.75, size = 0.7, color = "black", linetype = "solid") +
  ...
  if ("h2" %in% options_selected) {
    if (any(df$H2GramsPerDay > 0)) {
      p4 <- df %>%
        ...
        {
          ggplot(., aes(x = reorder(FarmName, -daily_H2), y = daily_H2, color = daily_H2)) +
            geom_boxplot(fatten = NULL, outlier.shape = NA) +
            stat_summary(fun = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..), width = 0.75, size = 0.7, color = "black", linetype = "solid") +
