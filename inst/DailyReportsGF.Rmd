...
**Number of animals with records: `r nrow(semi_join(CowsInExperiment, df, by = c("EID" = "RFID")))` out of `r nrow(CowsInExperiment)`**
...
if (any(df$H2GramsPerDay > 0)) {
  ...
  {ggplot(., aes(x = reorder(FarmName, -daily_H2), y = daily_H2, color = daily_H2)) + 
      ...
      geom_hline(yintercept = mean(.$daily_H2), linetype = "dashed", color = "yellow", linewidth = 0.6) +
