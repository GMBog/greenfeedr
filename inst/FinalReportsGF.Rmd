...
**Number of animals with gas records: `r if (!is.na(RFID_file) && nrow(RFID_file) > 0) { paste0(nrow(dplyr::semi_join(RFID_file, df, by = "RFID")), " out of ", nrow(RFID_file)) } else { length(unique(df$RFID)) }`**
...
group_var <- if (!is.na(RFID_file) && nrow(RFID_file) > 0) "FarmName" else "RFID"
