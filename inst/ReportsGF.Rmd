...
**Number of cows with records: `r nrow(semi_join(CowsInExperiment, df, by = c("EID" = "RFID")))` cows / Out of `r nrow(CowsInExperiment)`**

**List of cows in the study:**
...
plot2 <- df %>%
  ...
  {ggplot(., aes(x = reorder(FarmName, -daily_CH4), y = daily_CH4, color = daily_CH4)) + 
      ...
      geom_hline(yintercept = mean(.$daily_CH4), linetype = "dashed", color = "#3366FF", linewidth = 0.6) +
