...
Note that we received from C-Lock Inc. the finalized data (or Summarized Data) from our study using GreenFeed system. So, now we need to process all the daily records obtained.
...
To do this we will use the `process_gfdata()` function and test 3 threshold values that will define the records we will retain for further analysis. Note that the function includes 3 parameters:
  ...
  - param2 = number of days with records per week
  - min_time - minimum duration of a record
...
The next step, is to evaluate the function `process_gfdata()` with the defined set of parameters.
